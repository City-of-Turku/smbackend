...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("services", "0102_add_mobileunit_to_search_view"),
    ]
    operations = [
        migrations.RunSQL(
            sql="""
            ...
            UNION
            SELECT concat('contenttype_', id) AS id,  name_fi ,name_sv, name_en, search_column_fi, search_column_sv, search_column_en, 'ContentType' AS type_name from mobility_data_contenttype;
            """,
            reverse_sql="""
            CREATE OR REPLACE VIEW search_view as
            SELECT concat('unit_', services_unit.id) AS id, name_fi, name_sv, name_en, search_column_fi, search_column_sv, search_column_en, 'Unit' AS type_name from services_unit
