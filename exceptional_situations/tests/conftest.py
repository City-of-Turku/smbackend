...
def locations(announcements):
    ...
    SituationLocation.objects.create(
        details=json_data,
        geometry=GEOSGeometry("POINT(1 0)"),
...
@pytest.fixture
def invactive_locations(inactive_announcements):
    json_data = {"test_key": "test_value"}
    SituationLocation.objects.create(
        details=json_data,
        geometry=GEOSGeometry("POINT(0 0)"),
        announcement=inactive_announcements.first(),
    )
